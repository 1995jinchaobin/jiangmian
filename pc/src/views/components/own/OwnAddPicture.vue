  ...
  methods: {
    ...
    chooseFile () {
      ...
      if (files.files == null) {
        ...
        return;
      } else if (files.files.length > 10000) {
    ...
    getUpLoadUrl (file) {
      let self = this;
      setTimeout(function () {
        self.$post(self.requestURL.picbd, {
          file: file
        }).then((res) => {
          if (res.message == "上传成功") {
    ...
    deleteP (item) {
      this.$confirm('确认删除该图片吗?', '提示', {
        ...
        let self = this;
        this.$deletefn(this.requestURL.picbd, {
          ids: item.productId
        }).then((res) => {
          if (res.message == '删除失败') {
            // self.showInfo = 1;
